...
export function StudioClient() {
  ...
  async function startSession() {
    ...
    const started = await fetch(`${gatewayBase}/session/start`, { method: "POST", headers: { "Content-Type": "application/json", "x-user-id": "demo-user" }, body: JSON.stringify({ style: "default" }) }).then((r) => r.json());
  ...
  return (
    <main style={{ padding: 24 }}>
      ...
      <label>Avatar upload: <input type="file" accept="image/*" /></label>
      <div>
        <label htmlFor="style">Style: </label>
        <select id="style"><option>default</option><option>cinematic</option><option>anime</option></select>
      </div>
